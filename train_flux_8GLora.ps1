...
$train_data_dir = "./train/qinglong/train_sd3" # train dataset path | 训练数据集路径
...
$train_t5xxl = 0 #训练T5
$split_mode = 1 # 是否分离模式，默认为0, 开启后只训练单块减少显存加快训练速度。
...
$output_name = "flux-test-lokr" # output model name | 模型保存名称
...
$factor = 4 #只适用于lokr的因子，-1~8，8为全维度
...
elseif ($train_mode -ieq "flux_lora") {
  ...
  if ($split_mode -ne 0 -or $split_qkv -ne 0 -or $train_t5xxl -ne 0) {
    ...
    if ($train_t5xxl -ne 0 -and $enable_lycoris -ne 1) {
