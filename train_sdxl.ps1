...
$pretrained_model = "./models/sdxl/sdxl-v1.safetensors" # base model path | 底模路径
$model_type = "sdxl" # sd1.5 sd2.0 sdxl flux model | 可选 sd1.5 sd2.0 sdxl flux。SD2.0模型下 clip_skip 默认无效
...
foreach ($venvPath in $venvPaths) {
    ...
    if (Test-Path $activateScript -PathType Leaf) {
        try {
            ...
            Write-Warning "激活虚拟环境 $venvPath 时出错: $_"
        }
    }
}

if (-not $activated) {
    Write-Warning "未找到可激活的虚拟环境，将使用系统默认Python环境。"
    Write-Warning "已检查路径: $($venvPaths -join ', ')"
}
